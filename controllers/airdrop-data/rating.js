...
export async function ratingAidrop(req, res) {
  try {
    const client = await connectToDatabase();
    const database = client.db("airdrop");
    const collection = database.collection("users");

    const { _id, airdropId, rating } = req.body;

    if (!_id || !airdropId || !rating) {
      return res.status(400).json({
        message: "Missing required fields: _id, airdropId, rating",
      });
    }
    ...
    const result = await collection.updateOne(
      ...
      {
        $set: {
          "additionalAirdrop.$.rating": rating,
        },
      }
    );
